...
parser.add_argument('--output_dir', default="logs", type=str, help='output dir')
...
parser.add_argument('--net', default="swin_unet", choices=['unet', 'swin_unet'])
...
if __name__ == "__main__":
    ...
    if args.net == 'swin_unet':
        net = ViT_seg(config, img_size=args.img_size, num_classes=args.num_classes).cuda()
    else:
        net = Unet(num_classes=args.num_classes).cuda()

    args.output_dir += "/" + args.net
    snapshot = os.path.join(args.output_dir, 'best_model.pth')
    if not os.path.exists(snapshot):
        snapshot = snapshot.replace('best_model', 'epoch_' + str(150 - 1))
    msg = net.load_state_dict(torch.load(snapshot))
    print("self trained", msg)
